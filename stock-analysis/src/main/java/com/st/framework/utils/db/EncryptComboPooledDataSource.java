...
import com.st.framework.utils.encrypt.impl.EncryptorBase64Impl;

public class EncryptComboPooledDataSource extends AbstractPoolBackedDataSource implements PooledDataSource, Serializable, Referenceable{

	
	

    final static MLogger logger = MLog.getLogger( EncryptComboPooledDataSource.class );
    ...
    public EncryptComboPooledDataSource()
    { this( true ); }

    public EncryptComboPooledDataSource( boolean autoregister )
    {
        super( autoregister );
    ...
    public EncryptComboPooledDataSource(String configName)
    { 
        this();
...
//      System.err.println("getting jdbcUrl: " + dmds.getJdbcUrl()); 
    ...
    { 
    	...
    	jdbcUrl = base64.decrypt(jdbcUrl);
...
//      sb.append("; userOverrides: " + userOverrides.toString());
	...
	public static void main(String[] args) {
		String source = "appuser";
		EncryptorBase64Impl base64 = new EncryptorBase64Impl();
		String result = base64.encrypt(source);
		System.out.println("base64加密结果："+result);
	}
}
