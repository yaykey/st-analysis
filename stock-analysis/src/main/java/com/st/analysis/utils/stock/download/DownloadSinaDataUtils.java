...
public class DownloadSinaDataUtils extends BaseDBUtils {
	...
	final static Object daysOfflock = new Object();
	...
	public static void nioDownload(final String stockCode,
		...
		final DateFormat DF_SIMPLE = new SimpleDateFormat("yyyyMMdd");
		daysOff = factDateHolidayListMapper.selectDaysOff(DF_DAY.format(begin),
				DF_DAY.format(end));
		...
		List<Integer> successDays = gDetailManager.selectDetailActiveDateId(
				...
				Integer.parseInt(DF_SIMPLE.format(end)));
					...
					if (asyFlag == true) {
						tasks.add(new Callable<String>() {
							...
							public String call() throws Exception {
								try {
									createNioDownload(stockCode, stockType,
											fbegin).start();
								} catch (Exception ex) {
									logger.error(
											stockCode
...
//						});
						...
						createNioDownload(stockCode, stockType, fbegin).start();
					}
				}
			}
	...
	public static void downloadAllStockData() {
		List<DStock> stocks = null;
		{
			DStockExample example = new DStockExample();
			example.setOrderByClause("STOCK_CODE");

			DStockExample.Criteria c = example.createCriteria();
			//c.andStockTypeCodeEqualTo("sh");

			//c.andStockNameNotLike("%B%");
			
//			c.andStockCodeGreaterThan("000729");
			c.andStockCodeGreaterThan("000100");
			
			c.andStockCodeLessThanOrEqualTo("000149");

//			c.andStockCodeLessThan("300000");
			
//			DStockExample.Criteria orc = example.or();
//			orc.andStockCodeGreaterThan("399999");
//			orc.andStockTypeCodeEqualTo("sz");

			stocks = dStockManager.selectByExample(example);
		}


		for (DStock st : stocks) {
			nioDownload(st.getStockCode(), st.getStockTypeCode(), new Date(
					"2014/01/02"), new Date(), true);
		}
	}
	...
	public static void main(String[] args) {
		...
		// // for (int i = 39; i<=39; i++) {
		// nioDownload(DateUtils.frontCompWithZore(i, 6), "sz", null, null,
		// false);
		// try {
		// Thread.sleep(1000);
		// } catch (InterruptedException e) {
