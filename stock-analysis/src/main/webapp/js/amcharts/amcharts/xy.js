AmCharts.AmXYChart = AmCharts.Class({
	...
	createValueAxes : function() {
		...
		0 === c.length
				&& (d = new AmCharts.ValueAxis(this.theme), d.rotate = !1, d
						.setOrientation(!1), b.push(d), c.push(d));
		0 === a.length
				&& (d = new AmCharts.ValueAxis(this.theme), d.rotate = !0, d
						.setOrientation(!0), b.push(d), a.push(d));
		for (e = 0; e < b.length; e++)
			this.processValueAxis(b[e], e);
		a = this.graphs;
	...
	dispatchAxisZoom : function() {
		...
		for (c = 0; c < a.length; c++) {
			...
			if (!isNaN(b.min) && !isNaN(b.max)) {
				var d, e;
				"V" == b.orientation ? (d = b
						.coordinateToValue(-this.verticalPosition), e = b
						.coordinateToValue(-this.verticalPosition
								+ this.plotAreaHeight)) : (d = b
						.coordinateToValue(-this.horizontalPosition), e = b
	...
	updateData : function() {
		this.parseData();
		var a = this.chartData, c = a.length - 1, b = this.graphs, d = this.dataProvider, e = -Infinity, f = Infinity, k, g;
		...
		for (k = 0; k < b.length; k++)
			g = b[k], g.maxValue = e, g.minValue = f;
	...
	processGraph : function(a) {
		AmCharts.isString(a.xAxis)
				&& (a.xAxis = this.getValueAxisById(a.xAxis));
		AmCharts.isString(a.yAxis)
				&& (a.yAxis = this.getValueAxisById(a.yAxis));
	...
	formatString : function(a, c, b) {
		...
		-1 != a.indexOf("[[")
				&& (a = AmCharts.formatDataContextValue(a, c.dataContext));
	...
	addChartScrollbar : function(a) {
		...
		if (a) {
			...
			var c = "backgroundColor backgroundAlpha selectedBackgroundColor selectedBackgroundAlpha scrollDuration resizeEnabled hideResizeGrips scrollbarHeight updateOnReleaseOnly"
					.split(" ");
			if (!this.hideYScrollbar) {
			...
			this.hideXScrollbar
					|| (b = new AmCharts.SimpleChartScrollbar(this.theme), b.skipEvent = !0, b.chart = this, this
							.listenTo(b, "zoomed", this.handleHSBZoom), AmCharts
							.copyProperties(a, b, c), b.rotate = !1, this.scrollbarH = b)
		}
	},
	updateTrendLines : function() {
		var a = this.trendLines, c;
		for (c = 0; c < a.length; c++) {
			var b = a[c], b = AmCharts.processObject(b, AmCharts.TrendLine,
					this.theme);
			a[c] = b;
			b.chart = this;
			var d = b.valueAxis;
	...
	handleAxisSelfZoom : function(a) {
		if ("H" == a.valueAxis.orientation) {
			...
			c = this.fitMultiplier(a.multiplier), this.fitV(-a.position * c, c), this.heightMultiplier = c;
		this.zoomChart();
		a = this.graphs;
		for (c = 0; c < a.length; c++)
			a[c].setAnimationPlayed();
		this.zoomScrollbars()
	},
	handleCursorZoom : function(a) {
		var c = this.widthMultiplier * this.plotAreaWidth / a.selectionWidth, b = this.heightMultiplier
				* this.plotAreaHeight / a.selectionHeight, c = this
				.fitMultiplier(c), b = this.fitMultiplier(b), d = (this.verticalPosition - a.selectionY)
				* b / this.heightMultiplier;
		this.fitH((this.horizontalPosition - a.selectionX) * c
						/ this.widthMultiplier, c);
		this.fitV(d, b);
		this.widthMultiplier = c;
