AmCharts.AmSerialChart = AmCharts.Class({
	...
	initChart : function() {
		...
		a
				&& (a.updateData(), a.fullWidth
						&& (a.fullRectSet = this.cursorLineSet));
	...
	handleWheelReal : function(a, b) {
		if (!this.wheelBusy) {
			...
			0 > a
					? d
							? (l = this.endTime - this.startTime, d = this.startTime
									+ c * e, e = this.endTime + f * e, 0 < f
									&& 0 < c && e >= h && (e = h, d = h - l), this
									.zoomToDates(new Date(d), new Date(e)))
							: (0 < f && 0 < c && this.end >= l - 1
									&& (c = f = 0), d = this.start + c, e = this.end
									+ f, this.zoomToIndexes(d, e))
					: d
							? (l = this.endTime - this.startTime, d = this.startTime
									- c * e, e = this.endTime - f * e, 0 < f
									&& 0 < c && d <= g && (d = g, e = g + l), this
									.zoomToDates(new Date(d), new Date(e)))
							: (0 < f && 0 < c && 1 > this.start && (c = f = 0), d = this.start
									- c, e = this.end - f, this.zoomToIndexes(
									d, e))
		}
	},
	...
	updateData : function() {
		...
		for (b = 0; b < a.length; b++)
			a[b].data = c;
		0 < c.length
				&& (this.firstTime = this.getStartTime(c[0].time), this.lastTime = this
						.getEndTime(c[c.length - 1].time))
	},
	...
	getEndTime : function(a) {
		...
		return AmCharts.changeDate(new Date(a), b.period, b.count, !0)
				.getTime()
				- 1
	},
	...
	timeZoom : function(a, b) {
		...
		isNaN(c)
				|| (b != this.endTime && b - a > c
						&& (a = b - c, this.updateScrollbar = !0), a != this.startTime
						&& b - a > c && (b = a + c, this.updateScrollbar = !0));
	...
	indexZoom : function(a, b) {
		...
		isNaN(c)
				|| (b != this.end && b - a > c
						&& (a = b - c, this.updateScrollbar = !0), a != this.start
						&& b - a > c && (b = a + c, this.updateScrollbar = !0));
	...
	zoomScrollbar : function() {
		...
		a
				...
				&& (a.dragger.stop(), b.parseDates && !b.equalSpacing ? a
						.timeZoom(this.startTime, this.endTime) : a.zoom(
						this.start, this.end), this.updateScrollbar = !0)
	},
	updateTrendLines : function() {
		var a = this.trendLines, b;
		for (b = 0; b < a.length; b++) {
			var c = a[b], c = AmCharts.processObject(c, AmCharts.TrendLine,
					this.theme);
			a[b] = c;
			c.chart = this;
	...
	countColumns : function() {
		var a = 0, b = this.valueAxes.length, c = this.graphs.length, d, e, f = !1, l, h;
		for (h = 0; h < b; h++) {
			e = this.valueAxes[h];
			var g = e.stackType;
			if ("100%" == g || "regular" == g)
				for (f = !1, l = 0; l < c; l++)
					d = this.graphs[l], d.tcc = 1, d.valueAxis == e
							&& "column" == d.type
							&& (!f && d.stackable && (a++, f = !0), (!d.stackable
									&& d.clustered || d.newStack)
									&& a++, d.columnIndex = a - 1, d.clustered
									|| (d.columnIndex = 0));
			if ("none" == g || "3d" == g) {
				f = !1;
				for (l = 0; l < c; l++)
					d = this.graphs[l], d.valueAxis == e
							&& "column" == d.type
							&& (d.clustered
									? (d.tcc = 1, d.newStack && (a = 0), d.hidden
											|| (d.columnIndex = a, a++))
									: d.hidden
											|| (f = !0, d.tcc = 1, d.columnIndex = 0));
				f && 0 == a && (a = 1)
			}
			if ("3d" == g) {
				e = 1;
				for (h = 0; h < c; h++)
					d = this.graphs[h], d.newStack && e++, d.depthCount = e, d.tcc = a;
				a = e
			}
		}
		return a
	...
	zoomToIndexes : function(a, b) {
		...
		if (c) {
			var d = c.length;
			0 < d
					&& (0 > a && (a = 0), b > d - 1 && (b = d - 1), d = this.categoryAxis, d.parseDates
							&& !d.equalSpacing ? this.zoom(c[a].time, this
									.getEndTime(c[b].time)) : this.zoom(a, b))
		}
	},
	zoomToDates : function(a, b) {
		...
		if (this.categoryAxis.equalSpacing) {
			var d = this.getClosestIndex(c, "time", a.getTime(), !0, 0,
					c.length);
	...
	formatPeriodString : function(a, b) {
		if (b) {
			var c = ["value", "open", "low", "high", "close"], d = "value open low high close average sum count"
					.split(" "), e = b.valueAxis, f = this.chartData, l = b.numberFormatter;
			l || (l = this.nf);
			for (var h = 0; h < c.length; h++) {
				for (var g = c[h], k = 0, m = 0, p, v, r, t, n, w = 0, q = 0, x, u, z, y, B, D = this.start; D <= this.end; D++) {
					var s = f[D];
					if (s && (s = s.axes[e.id].graphs[b.id])) {
						if (s.values) {
							var A = s.values[g];
							if (this.rotate) {
								if (0 > s.x || s.x > s.graph.height)
									A = NaN
							} else if (0 > s.x || s.x > s.graph.width)
								A = NaN;
							if (!isNaN(A)) {
								isNaN(p) && (p = A);
								v = A;
								...
								var H = AmCharts.getDecimals(A), k = k + A, k = AmCharts
										.roundTo(k, Math.max(n, H));
								m++;
								n = k / m
							}
						}
						if (s.percents && (s = s.percents[g], !isNaN(s))) {
							isNaN(x) && (x = s);
							u = s;
							if (isNaN(z) || z > s)
								z = s;
	...
	formatString : function(a, b, c) {
		...
		if (-1 != a.indexOf("[[category]]")) {
			...
			if (this.categoryAxis.parseDates) {
				...
				-1 != a.indexOf("[[category]]")
						&& (f = AmCharts.formatDate(e, f, this), -1 != f
								.indexOf("fff")
		...
		(f = e.duration)
				...
				&& (e = AmCharts.formatDuration(b.values.value, f, "",
						e.durationUnits, e.maxInterval, d), a = a.replace(
						RegExp("\\[\\[value\\]\\]", "g"), e));
...
AmCharts.Cuboid = AmCharts.Class({
	construct : function(a, b, c, d, e, f, l, h, g, k, m, p, v, r, t, n) {
		...
		this.topRadius = n;
		this.pattern = t;
		(this.rotate = v) ? 0 > b && 0 === m && (m = 180) : 0 > c && 270 == m
				&& (m = 90);
	...
	draw : function() {
		...
		if (0 < Math.abs(e) || 0 < Math.abs(f))
			if (isNaN(n))
				s = q, q = AmCharts.adjustLuminosity(w, -.2), q = AmCharts
						.adjustLuminosity(w, -.2), x = AmCharts.polygon(b, [0,
								e, c + e, c, 0], [0, f, f, 0, 0], q, h, 1, k,
						0, p), 0 < m
						&& (H = AmCharts.line(b, [0, e, c + e], [0, f, f], k,
								m, g, r)), u = AmCharts.polygon(b, [0, 0, c, c,
								0], [0, d, d, 0, 0], q, h, 1, k, 0, p), u
						.translate(e, f), 0 < m
						&& (z = AmCharts
								.line(b, [e, e], [f, f + d], k, m, g, r)), y = AmCharts
						.polygon(b, [0, 0, e, e, 0], [0, d, d + f, f, 0], q, h,
								1, k, 0, p), B = AmCharts.polygon(b, [c, c,
								c + e, c + e, c], [0, d, d + f, f, 0], q, h, 1,
						k, 0, p), 0 < m
						&& (D = AmCharts.line(b, [c, c + e, c + e, c], [0, f,
										d + f, d], k, m, g, r)), q = AmCharts
						.adjustLuminosity(s, .2), s = AmCharts.polygon(b, [0,
								e, c + e, c, 0], [d, d + f, d + f, d, d], q, h,
						1, k, 0, p), 0 < m
						&& (A = AmCharts.line(b, [0, e, c + e], [d, d + f,
										d + f], k, m, g, r));
			else {
				var J, K, L;
				I
						? (J = d / 2, q = e / 2, L = d / 2, K = c + e / 2, E = Math
								.abs(d / 2), C = Math.abs(e / 2))
						: (q = c / 2, J = f / 2, K = c / 2, L = d + f / 2 + 1, C = Math
								.abs(c / 2), E = Math.abs(f / 2));
				F = C * n;
				G = E * n;
				.1 < C
						&& .1 < C
						&& (x = AmCharts.circle(b, C, w, h, g, k, m, !1, E), x
								.translate(q, J));
				.1 < F
						&& .1 < F
						&& (s = AmCharts.circle(b, F, AmCharts
										.adjustLuminosity(w, .5), h, g, k, m,
								!1, G), s.translate(K, L))
			}
		h = M;
		1 > Math.abs(d) && (d = 0);
		1 > Math.abs(c) && (c = 0);
		!isNaN(n) && (0 < Math.abs(e) || 0 < Math.abs(f))
				? (l = [w], l = {
					fill : l,
					stroke : k,
					"stroke-width" : g,
					...
					"fill-opacity" : h
						...
						? (h = "M0,0 L" + c + "," + (d / 2 - d / 2 * n), g = " B", 0 < c
								...
								: (h += "A" + F + "," + G + ",0,0,0," + c + ","
										+ (d - d / 2 * (1 - n)) + "L0," + d, h += "A"
										+ C + "," + E + ",0,0,1,0,0"), C = 90)
						: (g = c / 2 - c / 2 * n, h = "M0,0 L" + g + "," + d, AmCharts.VML
								? (h = "M0,0 L" + g + "," + d, g = " B", 0 > d
										&& (g = " A"), h += g
										+ Math.round(c / 2 - F) + ","
										+ Math.round(d - G) + ","
										+ Math.round(c / 2 + F) + ","
										...
										+ Math.round(c / 2 - C)
										+ ","
										+ Math.round(-E) + "," + c + ",0,0,0")
								: (h += "A" + F + "," + G + ",0,0,0,"
										+ (c - c / 2 * (1 - n)) + "," + d + "L"
										+ c + ",0", h += "A" + C + "," + E
										+ ",0,0,1,0,0"), C = 180), b = b
						.path(h).attr(l), b.gradient("linearGradient", [w,
								AmCharts.adjustLuminosity(w, -.3),
								AmCharts.adjustLuminosity(w, -.3), w], C), I
						? b.translate(e / 2, 0)
						: b.translate(0, f / 2))
				: b = 0 === d
						? AmCharts.line(b, [0, c], [0, 0], k, m, g, r)
						: 0 === c ? AmCharts
								.line(b, [0, 0], [0, d], k, m, g, r) : 0 < v
								? AmCharts
										.rect(b, c, d, l, h, g, k, m, v, p, r)
								: AmCharts.polygon(b, [0, 0, c, c, 0], [0, d,
												d, 0, 0], l, h, g, k, m, p, !1,
										r);
		c = isNaN(n) ? 0 > d ? [x, H, u, z, y, B, D, s, A, b] : [s, A, u, z, y,
				B, x, H, D, b] : I ? 0 < c ? [x, b, s] : [s, b, x] : 0 > d ? [
				x, b, s] : [s, b, x];
...
AmCharts.CategoryAxis = AmCharts.Class({
	...
	draw : function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			b = this.end - d + 1;
			var l = this.gridCountR, h = this.showFirstLabel, g = this.showLastLabel, k, m = "", p = AmCharts
					.extractPeriod(this.minPeriod);
			k = AmCharts.getPeriodDuration(p.period, p.count);
			var v, r, t, n, w, q;
			v = this.rotate;
			var x = this.firstDayOfWeek, u = this.boldPeriodBeginning, a = AmCharts
					.resetDateToMin(new Date(a[a.length - 1].time + 1.05 * k),
							this.minPeriod, 1, x).getTime(), z;
			...
			if (this.parseDates && !this.equalSpacing) {
				...
				var l = Math.ceil(this.timeDifference / r) + 5, D = m = AmCharts
						.resetDateToMin(new Date(this.startTime - r), e, v, x)
						.getTime();
				t == e && 1 == v && this.centerLabelOnFullPeriod
						&& (w = r * this.stepWidth);
				...
				q
						&& 1 < v
						&& (y = this.chooseMinorFrequency(v), B = AmCharts
								.getPeriodDuration(e, y));
				if (0 < this.gridCountR)
					for (b = d; b <= l; b++) {
						p = s + r * (b + Math.floor((D - s) / r)) - A;
						"DD" == e && (p += 36E5);
						p = AmCharts.resetDateToMin(new Date(p), e, v, x)
								.getTime();
						"MM" == e
								&& (q = (p - m) / r, 1.5 <= (p - m) / r
										&& (p = p
												- (q - 1)
												* r
												+ AmCharts.getPeriodDuration(
														"DD", 3), p = AmCharts
												.resetDateToMin(new Date(p), e,
														1).getTime(), A += r));
						k = (p - this.startTime) * this.stepWidth;
						q = !1;
						this.nextPeriod[t]
								&& (q = this.checkPeriodChange(
										this.nextPeriod[t], 1, p, m, t));
						z = !1;
						q && this.markPeriodChange
								? (q = this.dateFormatsObject[this.nextPeriod[t]], this.twoLineMode
										&& (q = this.dateFormatsObject[t]
												+ "\n" + q, q = AmCharts
												.fixBrakes(q)), z = !0)
								: q = this.dateFormatsObject[t];
						u || (z = !1);
						...
						if (b == d && !h || b == l && !g)
							m = " ";
						this.labelFunction
								&& (m = this.labelFunction(m, new Date(p),
										this, e, v, n).toString());
						this.boldLabels && (z = !0);
						n = new this.axisItemRenderer(this, k, m, !1, w, 0, !1,
								z);
						this.pushAxisItem(n);
						n = m = p;
						if (!isNaN(y))
							for (k = 1; k < v; k += y)
								this.gridAlpha = this.minorGridAlpha, q = p + B
										* k, q = AmCharts.resetDateToMin(
										new Date(q), e, y, x).getTime(), q = new this.axisItemRenderer(
										this, (q - this.startTime)
												* this.stepWidth), this
										.pushAxisItem(q);
						this.gridAlpha = a
					}
			} else if (!this.parseDates) {
				if (this.cellWidth = this.getStepWidth(b), b < l && (l = b), f += this.start, this.stepWidth = this
						.getStepWidth(b), 0 < l)
					for (u = Math.floor(b / l), y = this
							.chooseMinorFrequency(u), k = f, k / 2 == Math
							.round(k / 2)
							&& k--, 0 > k && (k = 0), l = 0, this.end - k + 1 >= this.autoRotateCount
							&& (this.labelRotation = this.autoRotateAngle), b = k; b <= this.end
							+ 2; b++) {
						n = !1;
						0 <= b && b < this.data.length
								? (t = this.data[b], m = t.category, n = t.forceShow)
								: m = "";
						if (q && !isNaN(y))
							if (b / y == Math.round(b / y) || n)
								b / u == Math.round(b / u)
										|| n
										|| (this.gridAlpha = this.minorGridAlpha, m = void 0);
							else
								continue;
						else if (b / u != Math.round(b / u) && !n)
							continue;
						k = this.getCoordinate(b - f);
						n = 0;
						"start" == this.gridPosition
								&& (k -= this.cellWidth / 2, n = this.cellWidth
										/ 2);
						x = !0;
						tickShift = n;
						"start" == this.tickPosition
								&& (tickShift = 0, x = !1, n = 0);
						if (b == d && !h || b == this.end && !g)
							m = void 0;
						Math.round(l / e) != l / e && (m = void 0);
						l++;
						D = this.cellWidth;
						v && (D = NaN);
						this.labelFunction && t
								&& (m = this.labelFunction(m, t, this));
						m = AmCharts.fixBrakes(m);
						z = !1;
						this.boldLabels && (z = !0);
						b > this.end && "start" == this.tickPosition
								&& (m = " ");
						n = new this.axisItemRenderer(this, k, m, x, D, n,
								void 0, z, tickShift, !1, t.labelColor);
						n.serialDataItem = t;
						this.pushAxisItem(n);
						this.gridAlpha = a
					}
				...
				m = AmCharts.resetDateToMin(new Date(this.startTime - r), e, v,
						x).getTime();
				...
				r = k;
				if (this.data[k].time != AmCharts.resetDateToMin(
						new Date(this.data[k].time), e, v, x).getTime())
					for (x = 0, z = m, b = k; b < w; b++)
						p = this.data[b].time, this.checkPeriodChange(e, v, p,
								z)
								&& (x++, 2 <= x && (r = b, b = w), z = p);
				q
						&& 1 < v
						&& (y = this.chooseMinorFrequency(v), AmCharts
								.getPeriodDuration(e, y));
				if (0 < this.gridCountR)
					for (b = k; b < w; b++)
						if (p = this.data[b].time, this.checkPeriodChange(e, v,
								p, m)
								&& b >= r) {
							...
							z = !1;
							q && this.markPeriodChange
									? (q = this.dateFormatsObject[this.nextPeriod[t]], z = !0)
									: q = this.dateFormatsObject[t];
							m = AmCharts.formatDate(new Date(p), q, c);
							if (b == d && !h || b == l && !g)
								m = " ";
							B
									? B = !1
									: (u || (z = !1), k - this.previousPos > this.safeDistance
											* Math.cos(this.labelRotation
													* Math.PI / 180)
											&& (this.labelFunction
													&& (m = this.labelFunction(
															m, new Date(p),
															this, e, v, n)), this.boldLabels
													&& (z = !0), n = new this.axisItemRenderer(
													this, k, m, void 0, void 0,
													void 0, void 0, z), x = n
													.graphics(), this
													.pushAxisItem(n), n = x
													.getBBox().width, AmCharts.isModern
													|| (n -= k), this.previousPos = k
													+ n));
							n = m = p
						} else
							isNaN(y)
									|| (this.checkPeriodChange(e, y, p, D)
											&& (this.gridAlpha = this.minorGridAlpha, k = this
													.getCoordinate(b
															- this.start), q = new this.axisItemRenderer(
													this, k), this
													.pushAxisItem(q), D = p), this.gridAlpha = a)
			}
			for (b = 0; b < this.data.length; b++)
				if (h = this.data[b])
					g = this.parseDates && !this.equalSpacing ? Math
							.round((h.time - this.startTime) * this.stepWidth
									+ this.cellWidth / 2) : this
							.getCoordinate(b - f), h.x[this.id] = g;
			h = this.guides.length;
			for (b = 0; b < h; b++)
				g = this.guides[b], u = u = u = a = d = NaN, y = g.above, g.toCategory
						&& (u = c.getCategoryIndexByValue(g.toCategory), isNaN(u)
								|| (d = this.getCoordinate(u - f), g.expand
										&& (d += this.cellWidth / 2), n = new this.axisItemRenderer(
										this, d, "", !0, NaN, NaN, g), this
										.pushAxisItem(n, y))), g.category
						&& (u = c.getCategoryIndexByValue(g.category), isNaN(u)
								|| (a = this.getCoordinate(u - f), g.expand
										&& (a -= this.cellWidth / 2), u = (d - a)
										/ 2, n = new this.axisItemRenderer(
										this, a, g.label, !0, NaN, u, g), this
										.pushAxisItem(n, y))), g.toDate
						&& (g.toDate instanceof Date
								|| (g.toDate = AmCharts.stringToDate(g.toDate,
										c.dataDateFormat)), this.equalSpacing
								? (u = c.getClosestIndex(this.data, "time",
										g.toDate.getTime(), !1, 0,
										this.data.length - 1), isNaN(u)
										|| (d = this.getCoordinate(u - f)))
								: d = (g.toDate.getTime() - this.startTime)
										* this.stepWidth, n = new this.axisItemRenderer(
								this, d, "", !0, NaN, NaN, g), this
								.pushAxisItem(n, y)), g.date
						&& (g.date instanceof Date
								|| (g.date = AmCharts.stringToDate(g.date,
										c.dataDateFormat)), this.equalSpacing
								? (u = c.getClosestIndex(this.data, "time",
										g.date.getTime(), !1, 0,
										this.data.length - 1), isNaN(u)
										|| (a = this.getCoordinate(u - f)))
								: a = (g.date.getTime() - this.startTime)
										* this.stepWidth, u = (d - a) / 2, n = "H" == this.orientation
								? new this.axisItemRenderer(this, a, g.label,
										!1, 2 * u, NaN, g)
								: new this.axisItemRenderer(this, a, g.label,
										!1, NaN, u, g), this.pushAxisItem(n, y)), (0 < d || 0 < a)
						&& (d < this.width || a < this.width)
						&& (d = new this.guideFillRenderer(this, a, d, g), a = d
								.graphics(), this.pushAxisItem(d, y), g.graphics = a, a.index = b, g.balloonText
								&& this.addEventListeners(a, g))
		}
		this.axisCreated = !0;
		c = this.x;
		f = this.y;
		...
		2 < c - this.previousHeight && this.autoWrap && !this.parseDates
	...
	choosePeriod : function(a) {
		...
		return this.timeDifference < b && 0 < a
				? d[a - 1]
				: c <= this.gridCountR ? d[a] : a + 1 < d.length ? this
						.choosePeriod(a + 1) : d[a]
	},
	...
	checkPeriodChange : function(a, b, c, d, e) {
		...
		return "DD" == a && "hh" != e
				&& c - b <= AmCharts.getPeriodDuration(a, d) ? !1 : c != b
				? !0
				: !1
	},
	...
	xToIndex : function(a) {
		...
		this.parseDates && !this.equalSpacing
				? (a = this.startTime + Math.round(a / e) - this.minDuration()
						/ 2, c = c.getClosestIndex(b, "time", a, !1,
						this.start, this.end + 1))
				: (this.startOnAxis || (a -= e / 2), c = this.start
						+ Math.round(a / e));
