AmCharts.AmRadarChart = AmCharts.Class({
	...
	drawChart : function() {
		...
		var a = this.updateWidth(), b = this.updateHeight(), c = this.marginTop
				+ this.getTitleHeight(), d = this.marginLeft, b = b - c
				- this.marginBottom;
...
AmCharts.RadAxis = AmCharts.Class({
	construct : function(a) {
		...
		if (b) {
			...
			var w = a.length, t;
			for (t = 0; t < w; t++) {
				var k = 180 - 360 / w * t, h = p + this.axisWidth
						* Math.sin(k / 180 * Math.PI), q = e + this.axisWidth
						* Math.cos(k / 180 * Math.PI);
				0 < m
						&& (h = AmCharts.line(b.container, [p, h], [e, q], d,
								m, c), this.set.push(h));
				if (n) {
					var u = "start", h = p + (this.axisWidth + f)
							* Math.sin(k / 180 * Math.PI), q = e
							+ (this.axisWidth + f)
							* Math.cos(k / 180 * Math.PI);
					if (180 == k || 0 === k)
						u = "middle", h -= 5;
...
AmCharts.RadItem = AmCharts.Class({
	construct : function(a, b, c, d, m, p, e, f) {
		...
		var F = a.y;
		e
				? (C = !0, isNaN(e.tickLength) || (k = e.tickLength), void 0 != e.lineColor
						&& (u = e.lineColor), isNaN(e.lineAlpha)
						|| (h = e.lineAlpha), isNaN(e.dashLength)
						|| (A = e.dashLength), isNaN(e.lineThickness)
						|| (q = e.lineThickness), !0 === e.inside && (D = !0), void 0 !== e.boldLabel
						&& (f = e.boldLabel))
				: c || (h /= 3, k /= 2);
		var G = "end", z = -1;
		D && (G = "start", z = 1);
		var v;
		C
				&& (v = AmCharts.text(r, c, g, n, l, G, f), v.translate(p
								+ (k + 3 + H) * z, b), d.push(v), this.label = v, x = AmCharts
						.line(r, [p, p + k * z], [b, b], w, t, q), d.push(x));
		b = Math.round(a.y - b);
		f = [];
		n = [];
		if (0 < h) {
			if ("polygons" == E) {
				s = a.data.length;
				for (l = 0; l < s; l++)
					g = 180 - 360 / s * l, f.push(b
							* Math.sin(g / 180 * Math.PI)), n.push(b
							* Math.cos(g / 180 * Math.PI));
				f.push(f[0]);
				n.push(n[0]);
				h = AmCharts.line(r, f, n, u, h, q, A)
			} else
				h = AmCharts.circle(r, b, "#FFFFFF", 0, q, u, h);
			h.translate(p, F);
			d.push(h)
		}
		if (1 == m && 0 < y && !e && "" !== c) {
			...
			if ("polygons" == E) {
				for (l = s; 0 <= l; l--)
					g = 180 - 360 / s * l, f.push(e
							* Math.sin(g / 180 * Math.PI)), n.push(e
							* Math.cos(g / 180 * Math.PI));
				s = AmCharts.polygon(r, f, n, B, y)
			} else
				s = AmCharts.wedge(r, 0, 0, 0, 360, b, b, e, 0, {
							fill : B,
							"fill-opacity" : y,
							stroke : "#000",
							"stroke-opacity" : 0,
...
AmCharts.RadarFill = AmCharts.Class({
	construct : function(a, b, c, d) {
		...
		if ("polygons" == a.gridType) {
			...
			for (g = 0; g < l; g++)
				f = 180 - 360 / l * g, d.push(m * Math.sin(f / 180 * Math.PI)), n
						.push(m * Math.cos(f / 180 * Math.PI));
			d.push(d[0]);
			n.push(n[0]);
			for (g = l; 0 <= g; g--)
				f = 180 - 360 / l * g, d.push(b * Math.sin(f / 180 * Math.PI)), n
						.push(b * Math.cos(f / 180 * Math.PI));
