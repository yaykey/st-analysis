		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
					...
					//维度为分组  ---请勿删除 
				    /* if(${requestScope.type==2}){
					   //获取分组标签<optgroup> id是一样的 在源码中有修改，将其checked对应的值传过来添加到 arr中
			           var groupSelect=$("input[id='ui-multiselect-client-option']") ;
			           if(groupSelect.length>0){
			               for(var i=0;i<groupSelect.length;i++){
			                  if(groupSelect[i].checked){
			                      client_Ids += groupSelect[i].value+",";
			                  }
			               }
			           }
		            }  */
	               ...
	               /*  //日或小时的timeType都为1
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getChartVoipWarn.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
