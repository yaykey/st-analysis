		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
		            ...
		            //维度
		            var clientMultis=$(client.multiselect('getChecked'));
		            var client_Ids = "";
					var client_Types = "";
		            var dimId_dimTypeId="";
		           
		            for(var i=0;i<clientMultis.length;i++){
		            	dimId_dimTypeId +=$(client.multiselect('getChecked')[i]).attr('value')+",";
		            	var dimIds=$(client.multiselect('getChecked')[i]).attr('value').split("_");
						client_Ids += dimIds[0]+",";
		           ...
		           //维度为空 页面提示
	                ...
	                if(timeType==1||timeType==11)
	                	...
	                	var urlLast = base + "/report/amcharts/getChartBehavior.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						"&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&odimTypeId=${odimTypeId}&odimId=${odimId}&indexId="+dIndex_Ids+
						"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&tdimTypeId=${tdimTypeId}&tdimId="+client_Ids+"&timeFlag=${timeFlag}&dayReportId=1&weeklyReportId=2&monthlyReportId=4";
//	                 alert(urlLast);
