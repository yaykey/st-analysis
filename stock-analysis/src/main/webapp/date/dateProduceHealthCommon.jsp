...
<script type="text/javascript">
		$(document).ready(
			...
			function hideToSearch(){
		        ...
		        var cityIds=city.find(":selected");
	 		...
	 		if(city.attr("disabled")!="disabled"){
	          ...
	          //指标
	         var dIndex_Ids = "";
	 		if(dIndex.attr("disabled")!="disabled"){
	 			for (var i=0; i<dIndexIds.length; i++) {
	 				 if(dIndexIds[i].value!=''){
	 		...
	 		var dimIds = "";
	 		var dimTypeAndId = "";
	 		if(client.attr("disabled")!="disabled"){
	 			for (var i=0; i<clientIds.length; i++) {
	 				 if(clientIds[i].value!=''){
	 					 dimTypeAndId += clientIds[i].value;
	 					 dimIds += clientIds[i].value.split('_')[0];
	 					 dimTypeIds += clientIds[i].value.split('_')[1];
	         ...
	         if(timeType==1)//日报
	         {
	 		 ...
	 		 jQuery.ajax({
	 			type : "POST",
	 	        url : base + "/report/amcharts/getProductHealthChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
	 			      "&province="+province_Ids+"&city="+city_Ids+"&carrId=${carrId}&dimTypeId="+dimTypeIds+"&dimId="+dimIds+"&indexId="+dIndex_Ids+
