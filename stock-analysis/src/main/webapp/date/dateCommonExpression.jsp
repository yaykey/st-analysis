		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
		            ...
		            var clientIds = dim.find(":selected");
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getExpressionChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						"&queryType=3&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&dimTypeId="+client_Types+"&dimId="+client_Ids+"&indexId="+dIndex_Ids+
						"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&dimTypeAndId="+dimId_dimTypeId,
						dataType : 'text',
						data : {
