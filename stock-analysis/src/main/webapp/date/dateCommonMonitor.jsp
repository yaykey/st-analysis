		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
					...
					var collect=root.find("#collect");
					
				   var dataTypeId =dataType.find(":selected")[0].value;	//数据类型
		    	   var timeType = time.find(":selected")[0].value;		//时间类型
		           var monitorIds=monitor.find(":selected");			//
		           var collectIds=collect.find(":selected");			//借口编码（指标）
				   ...
				   for (var i=0; i<monitorIds.length; i++) {
					   dimId_dimTypeId += monitorIds[i].value;
			       ...
			       if(dimId_dimTypeId==""){
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/monitor/getChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}"+
								     "&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
									"&timeType="+timeType+"&indexId="+dIndex_Ids+
									"&startTime="+startTime+"&endTime="+endTime+"&dimTypeAndId="+dimId_dimTypeId+
									"&queryType="+dataTypeId+"&province=86&carrId=-1",
						dataType : 'text',
						data : {
						},
