		...
		$(document).ready(
			...
			function hideToSearch(){
				...
				for (var i=0; i<dIndexIds.length; i++) {
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts_assess/getChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						"&province="+province_Ids+"&city=&carrId=-1&dimTypeId="+client_Types+"&dimId="+client_Ids+"&indexId="+dIndex_Ids+
						"&analysisId=${analysisId}&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&dimTypeAndId="+dimId_dimTypeId+"&queryType="+dataTypeId[0].value,
						dataType : 'text',
						data : {
