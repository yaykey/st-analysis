		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getVoipChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
									"&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&dimTypeId="+client_Types+"&dimId="+client_Ids+"&indexId="+dIndex_Ids+
									"&sysType=${sysType}&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&dimTypeAndId="+dimId_dimTypeId+"&queryType="+dataTypeId[0].value,
						dataType : 'text',
						data : {
						},
