...
<script type="text/javascript">
		$(document).ready(
			...
			function hideToSearch(){
				...
				for (var i=0; i<dDims.length; i++) {
					if(dDims[i].selected==true)
					{
						var v=dDims[i].value;
						var v1=v.split('_');
						dimIds+=v1[0]+',';
						dimTypeIds+=v1[1]+',';
						dimTypeAndId+=v1[0]+'_'+v1[1]+',';
					}
				}
				...
				dimTypeIds= dimTypeIds.substring(0,dimTypeIds.length-1);
		           ...
		           jQuery.ajaxSetup({ cache: true });
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getFaimlyChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
