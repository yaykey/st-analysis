	...
	<script type="text/javascript">
		$(document).ready(
			function(){
				...
				/* Ajax取数据完毕  */
				if(birtType==2||birtType==6||birtType==7||birtType==4||birtType==8||birtType==9||birtType==10||birtType==13||birtType==11){
						var op = $('#rpt_day_');
				...
				function calendar(val){
					if(val==1){
						...
						if(birtType==2||birtType==6||birtType==7||birtType==4||birtType==8||birtType==10||birtType==13||birtType==11){
							...
							var end_date = date_formart(dayEnd,'Ymd');
							/* 设置日/月输入框可见，周输入框不可见，并将日/月输入框清空  */
							$('#databirt_').val(start_date + "-" +end_date);
							 $('#databirt_').DatePicker({
								format:'Ymd',
								date: new Date(),
								current: dayEnd,
