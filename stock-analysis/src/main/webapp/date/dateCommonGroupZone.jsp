		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
					...
					var oneLevelDim=root.find("#oneLevelDim");
					var secondLevelDim=root.find("#secondLevelDim");
		            ...
		            var oneLevelDimIds = oneLevelDim.find("option:selected");
		            var secondLevelDimIds = secondLevelDim.find("option:selected");
		            ...
		            //二级维度
		            var clientMultis=$(secondLevelDimIds.multiselect('getChecked'));
		            var secondLevelDimIds_Ids = "";
					var secondLevelDimIds_Types = "";
		            var dimId_dimTypeId="";
		           
		            for(var i=0;i<secondLevelDimIds.length;i++){
		            	dimId_dimTypeId +=$(secondLevelDimIds.multiselect('getChecked')[i]).attr('value')+"_"+oneLevelDimIds[0].value+",";
		            	var dimIds=$(secondLevelDimIds.multiselect('getChecked')[i]).attr('value').split("_");
		            	secondLevelDimIds_Ids += dimIds[0]+",";
						secondLevelDimIds_Types = secondLevelDimIds_Types.replace(dimIds[1]+",","")+dimIds[1]+",";
		            }
		
		            //截取字符串
		            secondLevelDimIds_Types= secondLevelDimIds_Types.substring(0,secondLevelDimIds_Types.length-1);
		            secondLevelDimIds_Ids= secondLevelDimIds_Ids.substring(0,secondLevelDimIds_Ids.length-1);
			        dimId_dimTypeId= dimId_dimTypeId.substring(0,dimId_dimTypeId.length-1);
	                
		           //维度为空 页面提示
			        if(secondLevelDimIds_Ids.length==0){
			            	alert('维度为空，请重新操作！');
			            	return false;
			        }
		      ...
		      /*       //指标为空 页面提示
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getChartGroupZone.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
									"&zoneTypeShow=${zoneTypeShow}&zoneNameShow=${zoneNameShow}&zoneName=${zoneName}&zoneNameType=${zoneNameType}&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&dimTypeId="+secondLevelDimIds_Types+"&dimId="+secondLevelDimIds_Ids+"&indexId="+dIndex_Ids+
									"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&dimTypeAndId="+dimId_dimTypeId+"&oneLevelDimIds="+oneLevelDimIds[0].value+"&queryType="+dataTypeId[0].value,
						dataType : 'text',
						data : {
