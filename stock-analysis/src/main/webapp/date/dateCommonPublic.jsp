		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
		            ...
		            var dIndexType=$("#type").find('option:selected').val();
		            var acur;
		            //不同的指标类型，对应的精度不同，用户质量精度为整数（acur=0）,用户规模精度为取两位小数（acur=2）
		            if(dIndexType==263){
		            	acur=0;
		            }else if(dIndexType==262){
		            	acur=2;
		            }
		            //指标为空 页面提示
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getPublicChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur="+acur+"&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						"&carrId=-1&indexId="+dIndex_Ids+"&queryType=3&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType,
						//&dimTypeId="+client_Types+&dimId="+client_Ids+"+"&dimTypeAndId="+dimId_dimTypeId,
						dataType : 'text',
						data : {
