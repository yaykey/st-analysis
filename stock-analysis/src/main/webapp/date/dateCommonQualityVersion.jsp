		...
		$(document).ready(
			function(){
				...
				function hideToSearch(){
				    ...
				    var birtversion=root.find("#birtversion");
					
		        	var dataTypeId =dataType.find(":selected");
		        	var timeType = time.find(":selected")[0].value;
		        	var clientIds = client.find("option:selected")[0].value;
		            var birtversionIds = birtversion.find("option:selected");
		            ...
		            //维度
		            var birtversionMultis=$(birtversion.multiselect('getChecked'));
		            var birtversion_Ids = "";
					var birtversion_Types = "";
		            var dimId_dimTypeId="";
		           
		            for(var i=0;i<birtversionMultis.length;i++){
		            	dimId_dimTypeId +=clientIds+"_"+$(birtversion.multiselect('getChecked')[i]).attr('value')+",";
		            	var dimIds=$(birtversion.multiselect('getChecked')[i]).attr('value').split("_");
		            	birtversion_Ids += clientIds+"_"+dimIds[0]+",";
		            	birtversion_Types = birtversion_Types.replace(dimIds[1]+",","")+dimIds[1]+",";
		            }
		
		            //截取字符串
		            birtversion_Types= birtversion_Types.substring(0,birtversion_Types.length-1);
		            birtversion_Ids= birtversion_Ids.substring(0,birtversion_Ids.length-1);
			        dimId_dimTypeId= dimId_dimTypeId.substring(0,dimId_dimTypeId.length-1);

		           //维度为空 页面提示
			        if(birtversion_Ids.length==0){
			            	alert('版本为空，请重新操作！');
			            	return false;
			        }
					...
					jQuery.ajax({
						type : "POST",
						//url : base + "/report/amcharts/getChartQualityVersion.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						...
						url : base + "/report/amcharts/getChartQualityVersion.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
									"&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&indexId="+dIndex_Ids+
									"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&dimTypeAndId="+dimId_dimTypeId+"&queryType="+dataTypeId[0].value,
						dataType : 'text',
						data : {
