		...
		$(document).ready(
			...
			function hideToSearch(){
				...
				dimTypeIds= dimTypeIds.substring(0,dimTypeIds.length-1);
					...
					jQuery.ajax({
						type : "POST",
						 url : base + "/report/amcharts/getGroupAssChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
					      "&province="+province_Ids+"&city="+city_Ids+"&carrId="+operator_Ids+"&dimTypeId="+dimTypeIds+"&dimId="+dimIds+"&indexId="+dIndex_Ids+
