	...
	<script type="text/javascript">
		$(document).ready(
			...
			function hideToSearch(){
				...
				var exDim=root.find("#exDim");
				var markFlag=root.find("#markFlag");
				var vestFlag=root.find("#vestFlag");
				var operators=root.find("#operators");
				...
				var dataTypeId =dataType.find(":selected");
		            ...
		            var exDimIds=exDim.find(":selected")[0].value;
		            var vestFlagId=vestFlag.find(":selected")[0].value;
		            var markFlagId=markFlag.find(":selected")[0].value;
				...
				var client_Types = "";
		            ...
		            for(var i=0;i<clientIds.length;i++){
		            	dimId_dimTypeId +=clientIds[i].value+"_"+exDimIds+",";
				...
				//省份
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getNearTopicChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=${acur}&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						...
						+"&vestFlag="+vestFlagId+"&markFlag="+markFlagId,
						dataType : 'text',
						data : {
						},
