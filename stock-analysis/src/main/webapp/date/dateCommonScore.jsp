			...
			function(){
		     ...
		     function hideToSearch(){
				...
				for (var i=0; i<dIndexIds.length; i++) {
					...
					jQuery.ajax({
						type : "POST",
						url : base + "/report/amcharts/getScoreChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur=1&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}&refreshFlag=1"+
						"&province="+province_Ids+"&zoneType=${zoneType}&city="+city_Ids+"&carrId=-1&dimTypeId="+client_Types+"&dimId="+client_Ids+"&indexId="+dIndex_Ids+
						"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&queryType=1&dimTypeAndId="+dimId_dimTypeId,
						dataType : 'text',
						data : {
