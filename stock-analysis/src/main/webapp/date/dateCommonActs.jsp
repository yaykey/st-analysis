	...
	<script type="text/javascript">
		$(document).ready(
			function(){
				...
				$.ajax({
					type:"post",
					data:"year=" + new Date().getFullYear() + "&rptid=" +　rptid  ,// + "&actionType=" +　actionType
				...
				function hideToSearch(){
					siden = true;
					var root = $("#r-main");
				      var chartversion=root.find("#chartversion");
				      var dIndexType=root.find("#dIndexType");
				      var dIndex=root.find("#dIndex");
				      var functionType =root.find("#functionType");
				      var time=root.find("#time");
				      var itemIds = $("#functionIds").attr("value");
				      var clientTypeId = $("#clientType").val();
				      var queryType = $("#dataType").val();
			        	var timeType = time.find(":selected")[0].value;
			            var versionIds = chartversion.find("option:selected");
			            var dIndexTypeIds=dIndexType.find(":selected");
			            ...
			            var acur=$("#dIndex option:selected").attr("acur");
					...
					for (var i=0; i<versionIds.length; i++) {
						version_ids += versionIds[i].value;
					...
					jQuery.ajax({
						type : "POST",
						 url : base + "/report/amcharts_acts/getChart.feinno?vrtlId=${vrtlId}&tableName=${tableName}&acur="+acur+"&dataType=${dataType}&xmlType=${xmlType}&widgetId=${widgetId}&functionId=${functionId}"
				 			+"&itemId="+itemIds+"&version="+version_ids+"&clientTypeId="+clientTypeId+"&indexId="+dIndex_Ids
				 			+"&startTime="+startTime+"&endTime="+endTime+"&timeType="+timeType+"&queryType="+queryType,
						dataType : 'text',
						data : {
