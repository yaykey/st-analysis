...
<link rel="stylesheet" type="text/css" href="<c:url value='/manager/jsp/userbehavior/css/style.css'/>"/>
<link rel="stylesheet" type="text/css" href="<c:url value='/manager/jsp/userbehavior/js/easyui/easyui.css'/>"/>
<link rel="stylesheet" type="text/css" href="<c:url value='/manager/jsp/userbehavior/js/easyui/icon.css'/>"/>

<script type="text/javascript" src="<c:url value='/manager/jsp/userbehavior/js/easyui/jquery.easyui.min.js'/>"></script>
<script type="text/javascript" src="<c:url value='/manager/jsp/userbehavior/js/gridTable_report.js'/>"></script>
<script type="text/javascript" src="<c:url value='/manager/jsp/userbehavior/js/jquery.json-2.4.min.js'/>"></script>
<script type="text/javascript" src="<c:url value='/manager/jsp/userbehavior/js/jquery.form.js'/>"></script>
<script type="text/javascript" src="<c:url value='/js/jquery-blockUI/jquery.blockUI.js'/>"></script>
<script type="text/javascript" src="<c:url value='/manager/jsp/userbehavior/js/windowpop.js'/>"></script>
<script type="text/javascript" src="<c:url value='/js/utils/preview.js'/>"></script>
...
// 	alert(params.classId);
// 	alert(params.rptId);
// 	alert(params.itemId1);
// 	alert(params.groupId);
// 	alert(params.itemId2);
	...
	<div class="top_con">
		...
		<div class="func clearfix">
				...
				<p class="version clearfix">
					...
					<a href="javascript:void(-1);" id="chexiao" onclick="revokeFun();" style="margin-left:5px;" class="a1 fl">撤销版本</a>
			...
			<div class="inp_con clearfix">
				...
				<form name='f_form' id='f_form' target='frame' action='' enctype='multipart/form-data' method='post' >
				     <input id='f_file' name='importExcel' size="1" type='file'  onchange="inputfileToVisionInput();"
					     style='position:absolute; top:110; left:50px; height:30px;  filter:alpha(opacity:0);opacity: 0; ' 
...
$(document).ready(function() {
	...
	$("#add_vision button").bind('click', function(){//新增版本
		$.messager.confirm("提示","确定新建此版本，一旦创建将无法删除!",function(result){
			if(result){
				...
				jQuery.ajax({
					type : "POST",
					url : base + '/manager/userbehavior/operate/addVersion.'+actionExt,
					dataType : 'html',
					data : {
						"clientid" : clientid,
						"versionid":versionid,
	...
	$("#update_vision button").bind('click', function(){//修改版本号
		...
		jQuery.ajax({
			type : "POST",
			url : base + '/manager/userbehavior/operate/updateVersion.'+actionExt,
			dataType : 'html',
			data : {
				"clientid" : clientid,
				"versionid":versionid,
...
function changeClient(){
	...
	jQuery.ajax({
		type : "POST",
		url : base + '/manager/userbehavior/operate/versionQuery.'+actionExt,
		dataType : 'json',
		data : {
			"clientid" : clientid
		},
...
function revokeFun(){
	...
	if(newflag==0){
		$.messager.alert("提示","只能撤销已经提交过的版本!","info");
		return false;
	}
	...
	jQuery.ajax({
		type : "POST",
		url : base + '/manager/userbehavior/operate/revokeFun.'+actionExt,
		dataType : 'json',
		data : {
			"clientTypeId" : $("#clientid").val(),
			"clientVsn" : $("#versionid").val()
		...
		success : function(responseText) {
			...
			if(responseText.chk==0){
				$.messager.alert("提示","撤销成功!","info",function(){
					$("#versionid").find("option:selected").attr("newflag","0");
					changeVersion();
				});
			}else{
		...
		error:function(){
			closeRpt();
			$.messager.alert("提示","您没有撤销权限，撤销不成功!","error");
		}
	});
}
