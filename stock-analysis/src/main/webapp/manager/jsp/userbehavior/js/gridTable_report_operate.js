var ReportDataGrid = {
		...
		initialize:function(){/**默认参数*/
			...
			//this.defaults.width = $(window).width() - 170;	/**表格宽度*/
			this.defaults.width = $(window).width()-20;	/**表格宽度*/
			...
			for(var i=0;i<dataArr.length;i++){
				if(dataArr[i][1]==this.defaults.ITEM_LEVEL_ONE){
					treeArr[n] = {
							...
							"selItemExId":dataArr[i][16],
							"selItemExName":dataArr[i][17],
							
							"selItemEffDate":dataArr[i][18],
							"selItemExpDate":dataArr[i][19],
				...
				$('#'+that.defaults.tableid).treegrid({
					...
					hideColumn:[[
			         ...
			         columns:[[
		                   ...
		                   {field:'rangetype',title:'文字链',width:(that.defaults.width*0.1).toFixed(0),align:'center'},
...
//					field.rangetype = "单选设置内容";
				...
				if(field.status!=this.defaults.ITEM_STATUS_ADD||this.editflag==1){
					saveSelItem(field.id,field.datatype);
				}
				
			}
		},
		updateSelItemValue:function(opt){
			var field = $('#'+this.defaults.tableid).treegrid('getSelected');
			if(field!=null){
				field.selItemId =  opt.selItemId;
				field.selItemName = opt.selItemName;
				
				field.selItemExId = opt.selItemExId;
				field.selItemExName = opt.selItemExName;
				
				field.selItemEffDate = opt.selItemEffDate;
				field.selItemExpDate = opt.selItemExpDate;
			}
		}
