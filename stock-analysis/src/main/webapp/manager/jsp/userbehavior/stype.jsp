	...
	<div>
		<ul id="static_rpt_type" class="v4_btn clearfix">
			<c:forEach items="${requestScope.reportContentList}" var="reportContent">
				<li id="rptidMenu" rptid="${reportContent.rptId}" itemcount="${reportContent.itemcount}" imageFilename="${reportContent.imageFilename}" rptname="${reportContent.rptName}" onclick="typelist('${reportContent.rptId}');">
	...
	$(document).ready(function() {
		...
		$("#addSaveRpt").bind("click", function() {
			...
			$('#rptImgForm').ajaxSubmit({
				url :base + '/manager/userbehavior/saveReport.' + actionExt,
				dataType : "json",
				type:"POST",
		...
		$("#delReport").bind("click", function() {
			$.messager.confirm("提示","确定删除?",function(result){
				if(result){
					...
					jQuery.ajax({
						type : "POST",
						url : base + '/manager/userbehavior/delReport.' + actionExt,
						dataType : 'html',
						data : {
							"rptid" : rptId,
		...
		$("#saveRpt").bind("click", function() {
			...
			jQuery.ajax({
				type : "POST",
				url : base + '/manager/userbehavior/updateReport.' + actionExt,
				dataType : 'html',
				data : {
					"rptId" : rptId,
					"rptName" : rptName
