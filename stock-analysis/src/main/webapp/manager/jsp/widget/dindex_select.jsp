...
<script>
		...
		$(function(){
			...
			var frmGroupIds = window.opener.document.getElementById("dindexIds")
			var frmGroupNames = window.opener.document.getElementById("dindexNames")
	    ...
	    function returnValue(frmGroupIds, frmGroupNames) {
	    	...
	    	for ( var x = 0; x < oSelect.childNodes.length; x++) {
	    		...
	    		if(oSelect.childNodes[x].selected)
	    		{	
	    			frmGroupIds.value += oSelect.childNodes[x].value + ".1,";
	    			frmGroupNames.value += oSelect.childNodes[x].innerHTML + "_1\n";
	    		}else{
	    			frmGroupIds.value += oSelect.childNodes[x].value + ".0,";
	    			frmGroupNames.value += oSelect.childNodes[x].innerHTML + "_0\n";
	    		}
		...
		function userTreeInit(){
			jQuery.ajax({
				type : "POST",
				url : '${base}/manager/widgetDim/getAllDIndex.feinno?widgetDimId=${requestScope.widgetDimId}',
				dataType : 'text',
				data : {
...
// 		    	var selectNodeList = treeObj.getNodesByFilter(filter); // 查找节点集合
		        ...
		        for(var i = 0;i<selectNodeList.length;i++){
		        	...
		        	selectNodeListReal =selectNodeListReal+ "{id:"+id+",name:'"+name+"',value:'"+value+"'},"; 
		        }
	...
	<div id="content" >
		<table align="center" class="pop-window" valign="top" width="70%" height="150">
			<tr >
				...
				<td class="td-left" >
					<div align="left">
						<strong>结果集</strong><red>(需要把指标置为默认选择时请选择中相应指标)</red>
					</div>
				</td>
			</tr>
			<tr >
				...
				<td  class="td-left"  style="vertical-align:top">
					...
					<select name="lstSelected" multiple id="lstSelected" style="width:215px;height:373px;margin-top: 9px">
						<c:forEach  var="dimExtend" items="${requestScope.dimExtendList}">
							<option value="${dimExtend.id}.${dimExtend.indexId}" 
								${dimExtend.isSelected == "1" ? "selected":""} >${dimExtend.indexName}</option> 
						</c:forEach>
					</select>
