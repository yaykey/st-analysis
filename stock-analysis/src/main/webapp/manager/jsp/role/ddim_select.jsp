...
<script>
		...
		function userTreeInit(){
			jQuery.ajax({
				type : "POST",
				url : '${base}/manager/role/authorise/getAllDDim.feinno?roleId=${requestScope.roleId}',
				dataType : 'text',
				data : {
		...
		<table align="center" class="pop-window" valign="top" width="70%" height="150">
			...
			<tr >
				<td   class="td-right"  style="vertical-align:top">
					<div style="text-align:left;margin:5px;height: 15px;position: absolute;left: 165;top: 0px">
				...
				<td  class="td-left"  style="vertical-align:top">
					...
					<select name="lstSelected" multiple id="lstSelected" style="width:215px;height:373px;margin-top: 9px">
						<c:forEach  var="ddim" items="${requestScope.ddimList}">
							<option value="${ddim.dimTypeId}.${ddim.dimId}" >${ddim.dimName}</option> 
						</c:forEach>
					</select>
