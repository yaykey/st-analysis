...
<script>
    ...
    $(function(){
    	...
    	$('#btn_select').bind('click', function() {
    		popWindowCenter('${base}/manager/role/authorise/getRoleClientTree.feinno?roleId='+$("#roleId").attr("value"), '选择模块', 552, 522);
    	});
    })
    ...
    function cancel(){
		...
		$("#addlist").attr("action","${base}/manager/role/listAndClient.feinno");
		$("#addlist").submit();
	}
	...
	function submitbutton(){
		...
		$("#addlist").attr("action","${base}/manager/role/authorise/addClients.feinno");
		$("#addlist").submit();
	}
</script>
</head>
			...
			<tr>
				...
				<td nowrap class="td-left">
					<textarea name="moduleNames" id="moduleNames"
						cols="20" rows="10" readonly="readonly"><c:forEach  var="module" items="${requestScope.moduleList}">${module.dimName}<%out.print("\n");%></c:forEach>
					</textarea>
					<input type="hidden" name="moduleIds"
						id="moduleIds"
						value="<c:forEach  var="module" items="${requestScope.moduleList}">${module.dimId},</c:forEach>" />
					<input type="button" id="btn_select"	value="分配客户端">
				</td>
			</tr>
