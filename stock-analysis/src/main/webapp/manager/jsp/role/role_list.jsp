...
<head>
    ...
    <script>
		...
		function queryRole(){
			...
			$("#list").attr("action","${base}/manager/role/list.feinno");
			$("#list").submit();
		}
		...
		function updateRole(){
			if (checkboxCount(list.choiceItem) == 0) {
				...
				var dt = new Date();
				$("#list").attr("action","${base}/manager/role/edit.feinno?dt"+dt+"&roleName_=${empty requestScope.roleName_ ? '':requestScope.roleName_}");
				$("#list").submit();	
			}
		...
		function goPage(pageNum) {
			...
			if (isInteger(pageNum, "请使用整型！")) {
				list.action = "${base}/manager/role/list.${actionExt}?page="
						+ pageNum;
				list.submit();
			}
		}
...
<script type="text/javascript">
		$(document).ready(function(){
			$('#roleName_').bind('keyup', function(event){
				   if (event.keyCode=="13"){
					   var roleName_ = $("#roleName_").val();
					   if(roleName_=="请输入查询的功能名称"){
						   roleName_ = "";
						}
						$('#list').attr('action', '${base}/manager/role/list.feinno?roleName_='+roleName_);
						$('#list').submit();
				   }
				});
		});
         ...
         <c:choose>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr class="test">
							...
							<th  width="6%">分配用户</th>
							<th  width="10%">角色授权</th>
							<!-- <th  width="10%">分配维度维值</th> -->
						</tr>
							
							<c:forEach  var="role" items="${requestScope.roleList}">
								<tr class="tr_test" align="center"  onclick="return trSelect('${role.id}')" style="CURSOR: hand">
