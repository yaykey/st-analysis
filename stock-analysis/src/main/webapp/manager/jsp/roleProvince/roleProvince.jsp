...
<script>
    ...
    $(function(){
    	...
    	$('#btn_select').bind('click', function() {
    		popWindowCenter('${base}/manager/roleProvince/getRoleProvinceTree.feinno?roleId='+$("#roleId").attr("value"), '选择模块', 552, 522);
    	});
    })
    ...
    function cancel(){
		...
		$("#addlist").attr("action","${base}/manager/roleProvince/list.feinno");
		$("#addlist").submit();
	}
	...
	function submitbutton(){
		...
		$("#addlist").attr("action","${base}/manager/roleProvince/editProvinces.feinno");
		$("#addlist").submit();
	}
</script>
</head>
			...
			<tr>
				...
				<td nowrap class="td-left">
					<textarea name="moduleNames" id="moduleNames"
						cols="20" rows="10" readonly="readonly"><c:forEach  var="module" items="${requestScope.provinces}">${module.provinceName}<%out.print("\n");%></c:forEach>
					</textarea>
					<input type="hidden" name="moduleIds"
						id="moduleIds"
						value="<c:forEach  var="module" items="${requestScope.provinces}">${module.provinceId},</c:forEach>" />
					<input type="button" id="btn_select"	value="分配省份">
				</td>
			</tr>
