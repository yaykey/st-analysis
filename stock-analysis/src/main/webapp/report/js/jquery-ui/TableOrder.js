...
(function () {
    ...
    var _tableObj = null, _tbodyObj = null, _tBodyIndex = 0,_theadObj = null;
    ...
    **/
    function Sort(tdIndex, options) {
        ...
        if (trJqObjArr) {
            if (options.Toggle) {
                if(_initConfig.IsLazyMode && isExist_trJqObjArr){
                	trJqObjArr=Reverse(tdIndex, options);
    ...
    function Reverse(tdIndex, options){
    	var trJqObjArr = new Array();
    	var tdOrderValArr = GetOrderTdValueArray(trJqObjArr, tdIndex, options.ValAttr, options.DataType);
    	var sort_len = trJqObjArr.length - 1;
        var isExchanged = false, compareRes;
    	for (var i = 0; i < sort_len; i++) {
            isExchanged = false;
            for (var j = sort_len; j > i; j--) {
    ...
    **/
    function SetOrderItem(obj, index, options) {
        ...
        obj.bind("click", function () {
            ...
            this.className = orderSettings.Desc ? "up" : "down";
            for ( var i = 0; i < _sortList.length; i++) {
            	if(_sortList[i] != index){
            		_theadObj.find("th:eq("+_sortList[i]+")").attr("class","thead");
            	}
    ...
    //#region 对外公开
    var _api = {
        Init: function (obj, tBodyIndex, sortList, options) {
            ...
            _theadObj = _tableObj.find("thead:eq(" + _tBodyIndex + ")");
            _sortList = sortList;
            options = options || {};
            _initConfig = {
