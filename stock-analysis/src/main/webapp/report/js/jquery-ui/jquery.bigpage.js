		...
		this.ajaxpage = function(param){
			...
			this.applyBuildTable = function(){
				...
				function buildTable(){
					...
					if($table.config.order){
						order($table.config.orderList);
					}
				}
				function order(orderList){
					TableOrderOper.Init("dataTable", 0, orderList);
//		            TableOrderOper.SetOrder("oemtagName", 2, {  DataType: "int",Desc: true });
//		            TableOrderOper.SetOrder("oemtag", 3, {  DataType: "int" ,Desc: false});
				}
			}
